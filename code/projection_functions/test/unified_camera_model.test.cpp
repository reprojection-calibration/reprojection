...
TEST(ProjectionFunctionsUnifiedCameraModel, TestUnifiedCameraModelProject) {
    Eigen::Array<double, 5, 1> const intrinsics{600, 600, 360, 240, 0.1};
    Eigen::MatrixX3d const gt_points{{0, 0, 10}, {-360, 0, 600}, {360, 0, 600}, {0, -240, 600}, {0, 240, 600}};
    Eigen::MatrixX2d const gt_pixels{{intrinsics[2], intrinsics[3]},
    ...
    for (int i{0}; i < gt_points.rows(); ++i) {
        Eigen::Vector2d const pixel_i(UnifiedCameraModel::Project<double>(intrinsics, gt_points.row(i)));
