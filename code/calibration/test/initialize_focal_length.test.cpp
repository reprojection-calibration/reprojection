...
// WARN(Jack): Because I did not want to spend a bunch of time engineering test data here, this test does not really
// check much except that the functions are called. At some later data maybe we can engineer some test data that lets us
// check the internal looping logic of the implementation.
TEST(CalibrationInitializeFocalLength, TestInitializeFocalLength) {
    ...
    EXPECT_EQ(std::size(fs_parabola), 0);  // No successful attempts!
