...
std::vector<Vector3d> const test_so3{Vector3d{0, 0, 0},     //
                                     Vector3d{M_PI, 0, 0},  //
                                     Vector3d{0, M_PI, 0},  //
                                     Vector3d{0, 0, M_PI}};

std::vector<Matrix3d> const test_SO3{Vector3d{1, 1, 1}.asDiagonal(),    //
                                     Vector3d{1, -1, -1}.asDiagonal(),  //
                                     Vector3d{-1, 1, -1}.asDiagonal(),  //
                                     Vector3d{-1, -1, 1}.asDiagonal()};

TEST(GeometryLie, TestSo3Exp) {
    ASSERT_EQ(std::size(test_so3), std::size(test_SO3));

    for (size_t i{0}; i < std::size(test_so3); ++i) {
        Vector3d const so3_i{test_so3[i]};
        Matrix3d const SO3_i{geometry::Exp(so3_i)};

        EXPECT_TRUE(SO3_i.isApprox(test_SO3[i]));
    }
}
...
TEST(GeometryLie, TestSo3Log) {
    ...
    for (size_t i{0}; i < std::size(test_so3); ++i) {
        Matrix3d const SO3_i{test_SO3[i]};
