...
using ImageSampleSource = std::function<std::optional<std::pair<uint64_t, cv::Mat>>()>;

// [omega_x, omega_y, omega_z, acc_x, acc_y, acc_z]
using ImuSampleSource = std::function<std::optional<std::pair<uint64_t, std::array<double, 6>>>()>;
