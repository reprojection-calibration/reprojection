...
int main(int argc, char* argv[]) {
    ...
    // application::ImageSourceSignature already brings us a lot of the way there I think but it needs some more engineering to reach
    // the above goals.
    // TODO(Jack): Unify the application::ImageSourceSignature with the application/types ImageSourceSignatureSignature
    std::unique_ptr<application::ImageSource> const image_feed{std::make_unique<application::VideoCapture>(app_args->data_path)};
    ...
    ImageSourceSignature image_source{[&image_feed, &pseudo_timestamp]() -> std::optional<std::pair<uint64_t, cv::Mat>> {
