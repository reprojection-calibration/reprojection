...
class FeatureExtractionTestFixture : public StepTestFixture {
   protected:
    void SetUp() override {
        ...
        target_info_id_ = db_.GetOrCreateStep(StepType::TargetInfo, "").first;
        TargetInfo const target_info{TargetType::Aprilgrid3, 6, 8, 0.1, false};
        db_.TargetInfoInsert(target_info_id_, target_id_, target_info);
    }
...
TEST_F(FeatureExtractionTestFixture, TestFeatureExtractionStep) {
    ...
    StepId const step_id{db_.GetOrCreateStep(StepType::FeatureExtraction, "").first};
