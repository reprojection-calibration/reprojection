...
std::optional<Vector3d> EvaluateSpline(CubicBSplineC3 const& spline, std::uint64_t const t_ns,
    ...
    auto const normalized_position{spline.time_handler_.SplinePosition(t_ns, spline.Size())};
    ...
    if (derivative == DerivativeOrder::Null) {
        return T_Model::template Evaluate<double, DerivativeOrder::Null>(P, u_i, spline.time_handler_.delta_t_ns_);
    } else if (derivative == DerivativeOrder::First) {
        return T_Model::template Evaluate<double, DerivativeOrder::First>(P, u_i, spline.time_handler_.delta_t_ns_);
    } else if (derivative == DerivativeOrder::Second) {
        return T_Model::template Evaluate<double, DerivativeOrder::Second>(P, u_i, spline.time_handler_.delta_t_ns_);
    } else {
