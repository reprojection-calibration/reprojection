...
// poses we dynamically set num_control_points in the underlying spline to a multiple of num_samples. If we did not
// do this and num_control_points was less than num_samples we would see "saw tooth" like artifacts in the sampled
// frame poses. What value the multiple needs to be is not certain, but for a first try we heuristically found that
// 2*num_samples was sufficient to build a spline that we can sample num_samples from without artifacts.
CameraCalibrationData GenerateMvgData(int const num_samples, CameraModel const camera_model, ArrayXd const& intrinsics,
                                      ImageBounds const& bounds, bool const flat, uint64_t const timespan_ns) {
    ...
    for (auto const time_i : times) {
        auto const pose_t{se3_spline.Evaluate(time_i)};
