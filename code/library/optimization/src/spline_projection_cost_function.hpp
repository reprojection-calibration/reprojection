...
class SplineProjectionCostFunction_T {
   public:
    ...
    bool operator()(T const* const intrinsics_ptr, T const* const control_point_0_ptr,
        ...
        Eigen::Map<const Array6<T>> const p0(control_point_0_ptr);
        Eigen::Map<const Array6<T>> const p1(control_point_1_ptr);
        Eigen::Map<const Array6<T>> const p2(control_point_2_ptr);
        Eigen::Map<const Array6<T>> const p3(control_point_3_ptr);

        Eigen::Matrix<T, 6, spline::constants::order> control_points;
