...
CameraState IntrinsicInitialization::Load(SqlitePtr const db) const {
    auto const loaded_intrinsics{
        database::ReadIntrinsics(db, camera_info_.sensor_name, StepType(), camera_info_.camera_model)};
...
void IntrinsicInitialization::Save(CameraState const& intrinsics, SqlitePtr const db) const {
    database::InsertIntrinsics(db, camera_info_.sensor_name, StepType(), camera_info_.camera_model, intrinsics);
}
