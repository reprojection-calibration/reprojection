...
class SplineInitFixture : public StepTestFixture {
   protected:
    void SetUp() override {
        ...
        db_.CameraPosesInsert(pose_init_id_, targets_id, camera_id_, poses);
    }
...
TEST_F(SplineInitFixture, TestSplineInitStepRunner) {
    ...
    auto const result{db_.ControlPointsSelect(step_id, camera_id_)};
    EXPECT_EQ(std::size(result), 3978);  // Heuristic

    auto const result2{db_.SplineInfoSelect(step_id, camera_id_)};
...
TEST_F(SplineInitFixture, TestSplineInitStep) {
    ...
    auto const [step_id, _]{db_.GetOrCreateStep(StepType::SplineInit, "")};
