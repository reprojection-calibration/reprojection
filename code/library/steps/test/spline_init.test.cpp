...
TEST_F(SplineInitFixture, TestSplineInitStepRunner) {
    ...
    auto const result{database::ControlPointsSelect(db_.get(), step_id, cam_.camera_id)};
    EXPECT_EQ(std::size(result), 3978);  // Heuristic

    auto const result2{database::SplineInfoSelect(db_.get(), step_id, cam_.camera_id)};
...
TEST_F(SplineInitFixture, TestSplineInitStep) {
    ...
    EXPECT_EQ(step.Assets(), std::vector{cam_.camera_id});
