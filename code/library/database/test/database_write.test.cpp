...
TEST_F(SensorDatabaseFixture, TestWriteToDbCalibrationStep) {
    EXPECT_NO_THROW(AddStep(CalibrationStep::pose_initialization));
...
TEST_F(SensorDatabaseFixture, TestWriteToDbCalibrationStepUpsert) {
    EXPECT_NO_THROW(AddStep(CalibrationStep::pose_initialization, "1"));
    EXPECT_NO_THROW(AddStep(CalibrationStep::pose_initialization, "2"));
    EXPECT_NO_THROW(AddStep(CalibrationStep::pose_initialization, "3"));
}
...
TEST_F(SensorDatabaseFixture, TestWriteToDbCameraIntrinsic) {
    EXPECT_THROW(database::WriteToDb({testing_utilities::pinhole_intrinsics}, CameraModel::Pinhole,
                                     CalibrationStep::pose_initialization, sensor_name, db),
                 std::runtime_error);
...
TEST_F(SensorDatabaseFixture, TestWriteToDbPoseData) {
    ...
    EXPECT_THROW(AddPose(CalibrationStep::pose_initialization), std::runtime_error);

    AddStep(CalibrationStep::pose_initialization);

    EXPECT_NO_THROW(AddPose(CalibrationStep::pose_initialization));
}
...
TEST_F(SensorDatabaseFixture, TestWriteToDbReprojectionError) {
    ...
    EXPECT_NO_THROW(database::WriteToDb(data, CalibrationStep::pose_initialization, sensor_name, db));
}
