...
std::tuple<std::tuple<Matrix3d, CeresState>, Vector3d> CameraImuExtrinsicInitialization(
    ...
    Vector3d const gravity{InitializeGravity(so3_spline, imu_data, R_imu_co)};

    return {{R_imu_co, diagnostics}, gravity};
}
...
std::tuple<Matrix3d, CeresState> InitializeCameraImuRotation(CubicBSplineC3 const& so3_spline,
    ...
    for (uint64_t const timestamp_ns : imu_data | std::views::keys) {
        auto const omega_co_i{EvaluateSpline<So3Spline>(so3_spline, timestamp_ns, DerivativeOrder::First)};
        if (omega_co_i) {
...
Vector3d InitializeGravity(CubicBSplineC3 const& so3_spline, ImuMeasurements const& imu_data,
    ...
    PositionMeasurements aa_co_w;
    for (uint64_t const timestamp_ns : imu_data | std::views::keys) {
        auto const orientation_i{EvaluateSpline<So3Spline>(so3_spline, timestamp_ns, DerivativeOrder::Null)};
        if (orientation_i) {
    ...
    Vector3d const unit_a_w{g * a_w.colwise().mean().normalized()};
